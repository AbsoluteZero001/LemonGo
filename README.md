...
- **监控台**：Dashboard 指标与趋势、请求日志与链路详情、用户活跃度、模块责任监控、开发者责任视图。
...
│       ├── views/monitor/      监控台：Dashboard、请求日志、活跃度、模块/开发者监控
...
| 商品 | `GET /api/products` · `GET/POST /api/products` · `GET/PUT/DELETE /api/products/{id}` | 商品列表 / 新增 / 详情 / 更新 / 删除 |
...
| 监控 | `GET /api/monitor/dashboard` · `/request-logs` · `/request-logs/{requestId}` · `/users/activity` · `/modules` · `/developers` | 监控指标 / 请求日志 / 详情 / 活跃度 / 模块 / 开发者 |
